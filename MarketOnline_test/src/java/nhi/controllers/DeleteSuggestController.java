 ...
 * @author admin
 */
public class DeleteSuggestController extends HttpServlet {
...
private final String success = "admin.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
           ...
           String maker = request.getParameter("txtUsername");
            SuggestEventDAO dao = new SuggestEventDAO();
            RegistrationDAO daoRe = new RegistrationDAO();
            boolean checkRe = false;
             checkRe = daoRe.delete(maker);
            boolean check = false;
              check = dao.delete(maker);
            if(check == true){
                url = success;
            }else{
                request.setAttribute("ERROR", "Can't delete Suggest");
            }
        } catch (Exception e) {
            e.printStackTrace();
           // log("ERROR at DeleteSuggestController " +e.getMessage());
        }finally{
